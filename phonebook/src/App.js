...
const App = () => {
      ...
      if (window.confirm(`${personFound.name} is already added to phonebook, replace the old number with a new one`)){
        phoneService
          ...
          .then( data => {
            ...
            setPersons(persons.map( person => person.id !== personFound.id ? person : personUpdated))
          })
      }
    }
    else{
    ...
    if (window.confirm(`Delete ${person.name}?`)){
      phoneService
        ...
        .catch( error => {
          console.log(error)
          setStyle("error")
          setMessage(`Information on ${person.name} has already been removed from the server`)
          setTimeout( () => {
            setMessage(null)
            setPersons(persons.filter(p => p.id !== person.id))
          }, 1000)
  ...
  const namesToShow = persons.filter( person => person.name.toLocaleLowerCase().includes(newFilter.toLocaleLowerCase()) )
